...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        np.save(folder_path + 'loss_train.npy', np.array(train_loss_list))
        np.save(folder_path + 'loss_vali.npy', np.array(vali_loss_list))
        np.save(folder_path + 'loss_test.npy', np.array(test_loss_list))
    ...
    def test(self, setting, test=0):
        ...
        print('mse:{:.4f}, mae:{:.4f}, rmse:{:.4f}, mape:{:.4f}, r2:{:.4f}'.format(mse, mae, rmse, mape, r2))

        # 2. 写入 result_long_term_forecast.txt
        f = open("result_long_term_forecast.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{:.4f}, mae:{:.4f}, rmse:{:.4f}, mape:{:.4f}, r2:{:.4f}'.format(mse, mae, rmse, mape, r2))
        f.write('\n\n')
        f.close()

        # 3. 保存 .npy 数据供后续画图 (确保不再引用 inputs)
